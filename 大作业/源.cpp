		...
		else 
		{
			...
			else
			{
				for (int i = startIndex; i <= endIndex; i++)
				{
					...
					else if (tokenLexer[i].tokenType == questionmark)
					{
						int ti = i + 1;
						int num = 1;
						while (num != 0)
						{
							if (tokenLexer[ti].tokenType == questionmark)num++;
							else if (tokenLexer[ti].tokenType == colon)num--;
							ti++;
						}
						ti--;
						generateExpression_1(startIndex, i - 1, addressOfResult);
						int address_1 = Cmd.size();
						int addressOfResult_1 = addressOfResult;
						addressOfResult++;
						Command* tcmd;
						Cmd.push_back(tcmd);
						generateExpression_1(i + 1, ti - 1, addressOfResult);
						int address_2 = Cmd.size();
						int addressOfResult_2 = addressOfResult;
						int address_3 = address_2 + 1;
						addressOfResult++;
						Cmd.push_back(tcmd);
						Cmd.push_back(tcmd);
						int skipGoal_1 = Cmd.size();
						generateExpression_1(ti + 1, endIndex, addressOfResult);
						addressOfResult++;
						OpCopy* curOp = new OpCopy(addressOfResult - 1, 0, addressOfResult);
						Cmd.push_back(curOp);
						int skipGoal_2 = Cmd.size();
						OpCheckSkip* cur1Op = new OpCheckSkip(addressOfResult_1, 0, skipGoal_1, Cmd);
						OpCopy* cur2Op = new OpCopy(addressOfResult_2, 0, addressOfResult);
						OpSkipX* cur3Op = new OpSkipX(0, 0, skipGoal_2, Cmd);
						Cmd[address_1] = cur1Op;
						Cmd[address_2] = cur2Op;
						Cmd[address_3] = cur3Op;
						return;
					}
				}
				//?与：
